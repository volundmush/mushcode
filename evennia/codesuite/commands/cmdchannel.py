...
from .. lib.CommAddon.models import PlayCommConf,CharCommConf,ChanCommConf
...
class SystemSendToChannel(MuxCommand):
    ...
    def func(self):
        ...
        #Okay, let's check to see if the enactor is muzzled!
        if playconf.muzzled:
            if playconf.muzzled > utcnow():
                caller.msg("You have been gagged from channel '%s' by %s until %s" % (channel.db_key,playconf.muzzledby.key,playconf.muzzled.ctime()))
                return
        
        # Enforce Mode 2's 'only when IC' feature and call character specific data if so.
        if chanconf.displaymode == 2:
            if not self.isic:
                string = "You can only use channel '%s' while @ic."
                caller.msg(string % channel.db_key)
                return
            
        # Set the header and the color. Defaults to {n
        header = "<{%s%s{n>" % (chanconf.color,channel.db_key)

        # Determine 'name' of speaker and any titles.
        if chanconf.displaymode == 0:
            speaker = self.caller.key
            if self.isic:
                title = charconf.ctitle
            else:
                title = playconf.ptitle
        elif chanconf.displaymode == 1:
            speaker = self.player.key
            title = playconf.ptitle
        elif chanconf.displaymode == 2:
            speaker = self.character.key
            title = charconf.ctitle
        elif chanconf.displaymode == 3:
            title = playconf.ptitle
            if self.isic:
                speaker = "%s.%s" % (self.player.key,self.character.key)
            else:
                speaker = self.player.key
        else:
            speaker = self.caller.key

        sendtext = speak(speaker,msg,fancy=True)

        if chanconf.titles and title:
            msg = "%s %s %s " % (header,title,sendtext)
        else:
            msg = "%s %s " % (header,sendtext)
...
class CmdCRename(MuxCommand):
    ...
    def func(self):
        ...
        if not rhs:
            ...
            self.caller.msg(msghead(self.sysname,error=True) + string)
            return
        channel2 = Channel.objects.filter(db_key=rhs)
        if len(channel2):
        ...
        cemit(channel.db_key,"Channel was Renamed to '%s' by %s" % (rhs,self.player.key))
        channel.db_key = rhs
        channel.save()

...
CREATE OR REPLACE VIEW volv_story_personas AS
	SELECT p.persona_id,e.entity_name as persona_name,e.entity_objid as persona_objid,p.persona_parent,p.template_id,t.template_name,t.template_x_name,p.x_splat_id,x.x_splat_name,t.template_y_name,p.y_splat_id,y.y_splat_name,t.template_z_name,p.z_splat_id,z.z_splat_name FROM vol_story_personas AS p LEFT JOIN vol_entity AS e ON e.entity_id=p.persona_id LEFT JOIN vol_story_templates AS t ON p.template_id=t.template_id LEFT JOIN vol_story_x_splats AS x ON p.x_splat_id=x.x_splat_id LEFT JOIN vol_story_y_splats AS y ON p.y_splat_id=y.y_splat_id LEFT JOIN vol_story_z_splats AS z ON p.z_splat_id=z.z_splat_id;

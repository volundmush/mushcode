...
CREATE OR REPLACE VIEW volv_story_personas_template_census AS
	SELECT template_id,template_name,template_sort,GROUP_CONCAT(persona_objid ORDER BY persona_name SEPARATOR '|') AS persona_objids,GROUP_CONCAT(persona_id ORDER BY persona_name SEPARATOR '|') AS persona_ids,GROUP_CONCAT(CONCAT_WS('/',persona_id,persona_name) ORDER BY persona_name SEPARATOR '|') AS persona_combo FROM volv_story_personas WHERE character_is_approved=1 AND character_is_deleted=0 GROUP BY template_id ORDER BY template_sort;

CREATE OR REPLACE VIEW volv_story_personas_template_census_x AS
	SELECT template_id,template_name,template_sort,template_x_name,x_splat_id,x_splat_name,GROUP_CONCAT(persona_objid ORDER BY persona_name SEPARATOR '|') AS persona_objids,GROUP_CONCAT(persona_id ORDER BY persona_name SEPARATOR '|') AS persona_ids,GROUP_CONCAT(CONCAT_WS('/',persona_id,persona_name) ORDER BY persona_name SEPARATOR '|') AS persona_combo FROM volv_story_personas WHERE character_is_approved=1 AND character_is_deleted=0 GROUP BY template_id,x_splat_id ORDER BY template_sort,x_splat_id;
